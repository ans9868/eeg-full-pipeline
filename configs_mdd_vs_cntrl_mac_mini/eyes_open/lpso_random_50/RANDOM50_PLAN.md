# Eyes-Open Random-50 LPSO Plan

This directory is for the eyes-open MDD-vs-control `random50` LPSO configs.
...
Current available eyes-open subjects:
...
- all `control` eyes-open EDF paths
- all `mdd` eyes-open EDF paths
...
- [generate_shared_random50_folds.py](/Volumes/CrucialX6/Home/projects/eeg-full-pipeline/configs_mdd_vs_cntrl_mac_mini/eyes_open/lpso_random_50/generate_shared_random50_folds.py)

It does two useful things:

- prints the relevant `config-maker.py` function blocks for reference
- previews or writes the shared `L2` and `L6` fold banks for the MDD/control eyes-open dataset
