#!/usr/bin/env python
"""
Create a per-subject summary for UNIFORM experiments showing:
- Subject ID (1-65)
...
# Uniform experiments (12 uniform folds)
EXPERIMENTS = {
...
def extract_subject_ids_from_fold(fold_name):
    """Extract subject IDs from fold name like 'sub-3_sub-60' or 'sub-1_sub-2_sub-3_sub-37_sub-38_sub-39'."""
    import re
...
def load_per_subject_accuracies(exp_name, results_dir):
    ...
    # Structure: {subject: set of unique fold names}
    ...
    return subject_accuracies, subject_unique_folds

def create_per_subject_summary():
    """Create per-subject summary for all uniform experiments."""
    ...
    for exp_name, results_dir in EXPERIMENTS.items():
        ...
        # Load per-subject accuracies and unique fold counts
        subject_accuracies, subject_unique_folds = load_per_subject_accuracies(exp_name, results_dir)
        ...
        for subject_num in range(1, 66):
            ...
            if accuracies:
                ...
                n_folds = len(unique_folds)  # Count unique folds, not total observations
    ...
    combined_report = []
    combined_report.append("# Per-Subject Summary: Uniform Experiments (12 Uniform Folds)")
    combined_report.append("=" * 80)
    combined_report.append("")
    combined_report.append("\n\n".join(all_reports))
    
    output_file = output_dir / "per_subject_summary_uniform_experiments.md"
    with open(output_file, 'w') as f:
